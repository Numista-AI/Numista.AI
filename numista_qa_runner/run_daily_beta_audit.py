"""
Numista.AI -- Master Daily Beta Feedback Audit Orchestrator (Optimized v3)
Automates day-by-day testing with multi-worker concurrency and fast timeouts:
1. Ingests daily feedback folder(s).
...
4. Executes Playwright E2E checks in parallel (workers: 4, timeout: 15s) and reads test-results.json.
5. Verifies zero net mutation on eric.seaman@yahoo.com via SHA-256 digest integrity gate.
6. Emits DAILY_BETA_AUDIT_REPORT_<DATE>.md report with itemized Pass/Fail status and screenshots.
"""
import os
import sys
import json
...
def parse_playwright_json():
    """Reads and parses reports/test-results.json generated by Playwright."""
    test_status_map = {}
    ...
    try:
        ...
        print(f"[RESULTS PARSER] Could not parse test-results.json: {e}")
    return test_status_map
...
def run_daily_beta_audit(target_folder_or_date=None):
    ...
    # Phase 4: Auth State Generation & Parallel Playwright E2E Execution
    print(f"\n--- PHASE 4: PLAYWRIGHT PARALLEL E2E EXECUTION (4 Workers, 15s Timeout) ---")
    run_cmd("python scripts/generate_test_auth_state.py", cwd=numista_tests_dir)
    
    # Run with 4 concurrent workers and 15s timeout for fast execution
    pw_ok, pw_out, pw_err = run_cmd(
        "npx playwright test tests/daily_feedback_dynamic.spec.js --workers=4 --timeout=15000",
        cwd=numista_tests_dir
    )

    test_status_map = parse_playwright_json()
