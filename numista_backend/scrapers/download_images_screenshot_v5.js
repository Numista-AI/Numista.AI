...
const files = [
    { name: '1942_walking_liberty_obverse.jpg', wiki: 'File:Walking_Liberty_half_dollar_obverse.jpg' },
    ...
    { name: '1937_buffalo_nickel_obverse.png', wiki: 'File:Indian_Head_Buffalo_Obverse.jpg' }, // Updated to jpg
...
(async () => {
    ...
    for (const file of files) {
        ...
        try {
            await page.goto(wikiUrl, { waitUntil: 'networkidle', timeout: 60000 });
            
            // Wait for the full image to be loaded
            const selectors = [
                '#file img',
                '.fullImage img',
            ...
            for (const selector of selectors) {
                try {
                    element = await page.waitForSelector(selector, { timeout: 15000, state: 'visible' });
                    if (element) {
                        // Check if it's the real image and not a tiny icon
                        const box = await element.boundingBox();
                        if (box && box.width > 100 && box.height > 100) {
                            console.log(`  Found image with selector: ${selector} (${Math.round(box.width)}x${Math.round(box.height)})`);
                            break;
                        } else {
                            console.log(`  Selector ${selector} found but too small (${box ? box.width : 0}x${box ? box.height : 0})`);
                            element = null;
                        }
                    }
                } catch (e) {}
            ...
            if (element) {
                ...
                console.log(`  Failed: No large image found for ${file.wiki}`);
            }
        } catch (e) {
