...
def login_screen():
    ...
    st.markdown(f"""
        <div style='text-align: center;'>
            ...
            <h3 style='margin-top: -15px; color: #475569;'>BETA v3.5</h3>
        </div>
    """, unsafe_allow_html=True)
        ...
        if st.session_state.get('signup_mode'):
            ...
            if st.button("Create Account", type="primary", use_container_width=True):
                if not new_email or "@" not in new_email:
                    ...
                    # Create User Logic
                    success, msg = update_user_pin(new_email, new_pin)
                    if success:
                        st.balloons()
...
@st.dialog("Verify & Enrich Numismatic Data", width="large")
def render_enrich_modal(coin_data):
    st.markdown("### Verify & Enrich Numismatic Data")
    st.caption("AI-identified details are pre-filled below. Please correct any errors and add scholarly details.")
    
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("🏛️ AI Pre-fill (Identity)")
        country = st.text_input("Country", value=coin_data.get('country', 'United States'))
        denom = st.text_input("Denomination", value=coin_data.get('denomination', ''))
        year = st.text_input("Mintage Year", value=str(coin_data.get('year', '')))
        mint = st.text_input("Mint Mark", value=coin_data.get('mint', ''))
        
    with col2:
        st.subheader("📖 Manual Enrichment")
        edge = st.text_input("Edge Inscription", placeholder="e.g. E PLURIBUS UNUM")
        grade = st.text_input("Estimated Grade", value=coin_data.get('grade', ''), placeholder="e.g. MS-65, AU-50")
        price = st.text_input("Purchase Price", value="$0.00")
        retailer = st.text_input("Retailer", placeholder="e.g. Heritage Auctions")
        p_date = st.date_input("Purchase Date", value=datetime.today())

    st.divider()
    st.subheader("📜 Scholarly Notes & Heirloom History")
    preservation = st.text_area("Preservation Notes", placeholder="Capture the history and family significance of the heirloom...", height=150)
    
    if st.button("Finalize & Add to Collection", type="primary", use_container_width=True):
        try:
            user_email = st.session_state.get('user_email')
            if user_email and not st.session_state.get('guest_mode'):
                new_id = str(uuid.uuid4())
                full_coin = {
                    "id": new_id,
                    "Country": country,
                    ...
                    "Mint Mark": mint,
                    "Condition": grade,
                    "Edge Inscription": edge,
                    "Purchase Price": price,
                    "Retailer": retailer,
                    "Purchase Date": str(p_date),
                    "Preservation Notes": preservation,
                    "Numismatic Report": coin_data.get('report', ''),
                    "imageUrlObverse": coin_data.get('image_obverse', ''),
                    "imageUrlReverse": coin_data.get('image_reverse', ''),
                    "imageUrlObverse_gcs": coin_data.get('image_obverse_gcs', ''),
                    "imageUrlReverse_gcs": coin_data.get('image_reverse_gcs', ''),
                    "created_at": firestore.SERVER_TIMESTAMP,
                    "deep_dive_status": "PENDING"
                }
                db.collection(f"users/{user_email}/coins").document(new_id).set(full_coin)
                st.success("✅ Coin successully added to your curated collection!")
                st.balloons()
                time.sleep(2)
                st.rerun()
            else:
                st.error("🔒 Guest Mode or User Email missing. Cannot save.")
        except Exception as e:
...
def render_microscope_agent():
    ...
    # Gold & Slate Theme HTML/JS Component (v3.2)
    html_code = """
    <div id="micro-container" style="background-color: #1e293b; border: 2px solid #d4af37; border-radius: 12px; padding: 25px; color: #f8fafc; font-family: 'Inter', sans-serif;">
        <h2 style="color: #d4af37; margin-top: 0;">🔬 Microscope Agent Status</h2>
        
        <nav style="display: flex; gap: 20px; align-items: center; margin-bottom: 20px;">
            <button id="scanBtn" style="background-color: #d4af37; color: #000; border: none; padding: 12px 24px; font-weight: bold; border-radius: 8px; cursor: pointer; flex: 1; text-transform: uppercase;">🚀 Connect & Scan</button>
            <div id="statusIndicator" style="flex: 2; font-size: 0.9rem; color: #94a3b8;">Click 'Connect & Scan' to begin.</div>
        </nav>

        <div id="focusSection" style="margin-top: 20px;">
            <div style="font-size: 0.8rem; color: #94a3b8; margin-bottom: 5px;">Focus Meter (Floor: 2000)</div>
            <div style="background: #334155; height: 12px; border-radius: 999px; overflow: hidden;">
                <div id="meterFill" style="height: 100%; width: 0%; background-color: #ef4444; transition: width 0.3s ease;"></div>
            </div>
            <div id="sharpnessVal" style="font-size: 0.7rem; text-align: right; color: #94a3b8; margin-top: 4px;">0 / 0</div>
        </div>

        <div id="inventoryWorkflow" style="display: none; margin-top: 25px; border-top: 1px solid #334155; padding-top: 20px;">
            <h3 style="color: #d4af37; margin-top: 0;">Gemini AI Report</h3>
            
            <!-- Scrollable Report Content -->
            <div id="reportScrollArea" style="max-height: 250px; overflow-y: auto; background: rgba(15, 23, 42, 0.4); border: 1px solid #334155; padding: 15px; border-radius: 8px; font-size: 0.85rem; color: #cbd5e1; margin-bottom: 20px; line-height: 1.5; font-style: italic;">
                <div id="repText">Analyzing...</div>
            </div>

            <div style="background: rgba(212, 175, 55, 0.05); border: 1px solid #d4af37; padding: 15px; border-radius: 8px;">
                <h4 style="margin: 0 0 10px 0; font-size: 0.9rem; color: #d4af37;">Confirm Coin Details</h4>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                    <div>
                        <label style="font-size: 0.7rem; color: #94a3b8; display: block;">Country</label>
                        <input id="editCountry" style="width: 100%; background: #0f172a; border: 1px solid #334155; color: #fff; border-radius: 4px; padding: 5px;">
                    </div>
                    <div>
                        <label style="font-size: 0.7rem; color: #94a3b8; display: block;">Year</label>
                        <input id="editYear" style="width: 100%; background: #0f172a; border: 1px solid #334155; color: #fff; border-radius: 4px; padding: 5px;">
                    </div>
                    <div>
                        <label style="font-size: 0.7rem; color: #94a3b8; display: block;">Mint Mark</label>
                        <input id="editMint" style="width: 100%; background: #0f172a; border: 1px solid #334155; color: #fff; border-radius: 4px; padding: 5px;">
                    </div>
                    <div>
                        <label style="font-size: 0.7rem; color: #94a3b8; display: block;">Grade</label>
                        <input id="editGrade" style="width: 100%; background: #0f172a; border: 1px solid #334155; color: #fff; border-radius: 4px; padding: 5px;">
                    </div>
                </div>
                <button id="saveBtn" style="margin-top: 15px; width: 100%; background-color: #fbbf24; color: #000; border: none; padding: 10px; border-radius: 6px; font-weight: bold; cursor: pointer;">✨ Add to My Collection</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const inventoryArea = document.getElementById('inventoryWorkflow');
        ...
        scanBtn.onclick = async () => {
            try {
                ...
                statusInd.innerText = "Connecting to Agent...";
                inventoryArea.style.display = 'none';
                
                const resp = await fetch(`${API_BASE}/start-scan`, { method: 'POST' });
                const data = await resp.json();
                
                if(data.status === 'success') {
                    statusInd.innerText = "Scan Started!";
                    startPolling();
                } else {
                    statusInd.innerText = "Error: " + data.message;
                    scanBtn.disabled = false;
                }
            } catch (e) {
                statusInd.innerHTML = "<span style='color: #ef4444;'>Connect Failed!</span> Ensure auto_capture.py is running.";
                scanBtn.disabled = false;
            }
        };
        ...
        saveBtn.onclick = async () => {
            ...
            const finalCoin = {
                ...
                mint: document.getElementById('editMint').value,
                grade: document.getElementById('editGrade').value,
                report: document.getElementById('repText').innerText
            };
            try {
                // LOCAL SYNC: Save to local CSV/Archive
                const localResp = await fetch(`${API_BASE}/add-to-collection`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(finalCoin)
                });
                const localData = await localResp.json();
                console.log("Local Save:", localData);
                
                // Merge GCS URIs if returned
                if (localData.gcs_obverse) finalCoin.image_obverse_gcs = localData.gcs_obverse;
                if (localData.gcs_reverse) finalCoin.image_reverse_gcs = localData.gcs_reverse;
            } catch (e) {
                console.error("Local Save Failed:", e);
            }

            // CLOUD ENRICHMENT: Trigger Streamlit Modal via safe Unicode-aware base64
            const jsonStr = JSON.stringify(finalCoin);
            const encoded = btoa(encodeURIComponent(jsonStr).replace(/%([0-9A-F]{2})/g, function(match, p1) {
                return String.fromCharCode('0x' + p1);
            }));
            window.parent.location.href = window.parent.location.pathname + "?enrich_micro_coin=" + encoded;
        };
        ...
        function startPolling() {
            ...
            poller = setInterval(async () => {
                try {
                    const r = await fetch(`${API_BASE}/get-status`);
                    const d = await r.json();
                    
                    statusInd.innerText = `${d.current_step}: ${d.status_message}`;
                    
                    // Update Meter
                    const pct = Math.min(100, (d.sharpness / (d.max_sharpness || 1)) * 100);
                    meterFill.style.width = pct + '%';
                    sharpnessVal.innerText = `${d.sharpness} / ${d.max_sharpness}`;
                    
                    // Meter Color
                    if(d.sharpness >= 2000) {
                        meterFill.style.backgroundColor = (d.status_message === 'READY!') ? '#10b981' : '#fbbf24';
                    } else {
                        meterFill.style.backgroundColor = '#ef4444';
                    }

                    if(d.error) {
                        statusInd.innerText = "Error: " + d.error;
                        clearInterval(poller);
                        scanBtn.disabled = false;
                    }

                    if(!d.is_active && d.last_report) {
                        currentCoinData = d.last_report;
                        document.getElementById('repText').innerText = d.last_report.report;
                        document.getElementById('editCountry').value = d.last_report.country || "";
                        document.getElementById('editYear').value = d.last_report.year || "";
                        document.getElementById('editMint').value = d.last_report.mint || "";
                        document.getElementById('editGrade').value = d.last_report.grade || "";
                        
                        inventoryArea.style.display = 'block';
                        statusInd.innerText = "Analysis Ready!";
                        clearInterval(poller);
                        scanBtn.disabled = false;
                    }
                } catch (e) {
                    clearInterval(poller);
                    statusInd.innerText = "Connection lost.";
                    scanBtn.disabled = false;
                }
            }, 1000);
        }
...
if not check_login():
    ...
    # --- MICROSCOPE ENRICHMENT BRIDGE (V3.5) ---
    enrich_save = st.query_params.get("enrich_micro_coin")
    if enrich_save:
        try:
            import json, base64, urllib.parse
            # Use urllib.parse.unquote for Unicode safety matching the JS side
            decoded_raw = base64.b64decode(enrich_save).decode('latin-1')
            json_str = urllib.parse.unquote(decoded_raw)
            coin_data = json.loads(json_str)
            # Clear query params to prevent re-triggering modal on refresh 
            st.query_params.clear() 
            # Set to session state and trigger rerun to open dialog in a clean state
            st.session_state['pending_micro_enrichment'] = coin_data
            st.rerun()
        except Exception as e:
            st.error(f"Enrichment Bridge Failed: {e}")

    # Open the Dialog if state is set
    if st.session_state.get('pending_micro_enrichment'):
        data = st.session_state.pop('pending_micro_enrichment')
        render_enrich_modal(data)
        ...
        VERSION_HISTORY = [
            {
                "version": "v3.5",
                "date": "2026-03-24",
                "desc": "Microscope Optimization & Auto-Crop",
                "changes": [
                    "Implemented 1080p high-resolution frame acquisition for Tomlov DM9.",
                    "Added resolution-independent dynamic UI for centering coins.",
                    "Implemented automatic 1080x1080 square cropping for professional archiving.",
                    "Optimized capture stability and motion detection thresholds."
                ]
            },
            {
                "version": "v3.4",
                "date": "2026-03-22",
                "desc": "Advanced Enrichment & Heirloom Tracking",
                "changes": [
                    "Implemented @st.dialog enrichment modal for scholarly coin data.",
                    "Added auto-trigger mechanism for hardware scan transitions.",
                    "Enhanced data schema for heirloom tracking (Edge Inscriptions, Preservation Notes).",
                    "Unified AI-prefill with manual corrections in a professional desktop layout."
                ]
            },
            {
                "version": "v3.3",
                "date": "2026-03-21",
                "desc": "Local Agent Sync & Verification Engine",
                "changes": [
                    "Integrated 'Microscope Agent' local-to-cloud synchronization.",
                    "Implemented physical image archiving to 'verified_images/' for live captures.",
                    "Added automated CSV logging to 'numista_master_log.csv' upon collection additions.",
                    "Enhanced cross-origin communication between Numista.AI and local agent."
                ]
            },
            {
                "version": "v3.2",
                "date": "2026-03-10",
                "desc": "Inventory Workflow Finalized",
                "changes": [
                    "Added scrollable numismatic reports to the microscope dashboard.",
                    "Implemented editable verification fields for Country, Year, Mint, and Grade.",
                    "Integrated 'Add to My Collection' button with live Firestore sync.",
                    "Added auto-clearing state for seamless batch scanning."
                ]
            },
            {
                "version": "v3.1",
                "date": "2026-03-10",
                "desc": "Microscope Hardware Agent Integration",
                "changes": [
                    "Integrated 'Hardware Agent' for high-resolution microscope capture.",
                    "Added real-time 'Focus Meter' and hardware status monitoring to the web interface.",
                    "Implemented 'Nano Banana 2' (Gemini 3.1 Flash) for professional numismatic reporting.",
                    "Enhanced Slate & Gold dashboard aesthetic for estate planning tools."
                ]
            },
            {
                "version": "v2.7",
                "date": "2026-03-07",
                "desc": "Improved UI Labels & Professional ID System",
                "changes": [
                    "Replaced cryptic 4-character hex IDs in 'Select Coin to Inspect' with professional, context-rich labels.",
                    "Implemented Option 3 labeling: '{Year}-{MintMark} {Denomination} | {Theme/Subject}'.",
                    "Enhanced visual scannability for large collections.",
                    "Verified system stability with 1891 Morgan Silver Dollar health check."
                ]
            },
            {
                "version": "v2.6",
    ...
    # --- 3. MY COLLECTION ---
        ...
        if df.empty:
            ...
            def get_professional_label(coin_id):
                ...
                label = f"{year_mint} {d}".strip()
                if s:
                    label += f" | {s}"
                return label

            selected_coin_name = st.selectbox("Select Coin to Inspect:", options=view_df['id'].tolist(), format_func=get_professional_label)
        ...
        render_add_excel()
    elif selection == 'ADD_MICROSCOPE':
        render_microscope_agent()
            ...
            if st.form_submit_button("Send Message", type="primary"):
                if not user_msg.strip():
                    ...
                    try:
                        ...
                        st.success("Thank you for your message! It has been sent successfully. If you are logged in, you may receive a response to your registered email.")
                    except Exception as e:
            ...
            if st.form_submit_button("Submit"):
                if not feedback_message.strip():
                    ...
                    try:
                        ...
                        st.success("Thank you for your feedback! It has been submitted successfully. If you are logged in, you may receive a response to your registered email.")
                        st.balloons()
                    except Exception as e:
