...
async def scrape_gaps_cron(limit: int = 50, target: str = "all", mode: str = "request", dry_run: bool = False, priority: str = "all"):
    ...
    try:
        ...
        # Read the report generated by the agent
        report_path = Path(__file__).parent / "sourcing_audit_report.md"
        report_content = ""
        if report_path.exists():
            with open(report_path, "r", encoding="utf-8") as f:
                report_content = f.read()
        
        # Save the report to Firestore under collection 'scraper_reports'
        ts = int(time.time())
        doc_id = f"report_{ts}"
        db.collection("scraper_reports").document(doc_id).set({
            "timestamp": ts,
        ...
        return {
            ...
            "processed_coins": processed_coins,
            "processed_errors": processed_errors,
            "report_saved_to_firestore": doc_id
        }
    except Exception as e:
...
async def get_gap_stats():
    ...
    """
    try:
        # Use the standard google-cloud-firestore client already imported
        db_cloud = firestore.Client(project=PROJECT_ID)
        
        # Check for cached stats first
        stats_doc = db_cloud.collection("config").document("stats").get()
        if stats_doc.exists:
            return stats_doc.to_dict()

        # Fallback to direct count if no cached stats
        ref_col = db_cloud.collection("definitive_reference")
        total_items = 9678 # Current known library size
        
        # Optimization: Counting in Firestore can be expensive. 
        # For now, we query and count lengths (acceptable for < 10k items)
        gaps_empty = [d for d in ref_col.where("image_url_obverse", "==", "").stream()]
        gaps_null = [d for d in ref_col.where("image_url_obverse", "==", None).stream()]
