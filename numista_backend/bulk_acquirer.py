...
# User-provided cookies that bypass 403
...
PAGES_TO_SCRAPE = [
    ...
    'https://www.usmint.gov/coins/coin-programs/american-innovation-dollar-coins',
    'https://www.usmint.gov/coins/coin-programs/native-american-coin-programs'
]
...
def get_page_links():
    ...
    for page_url in PAGES_TO_SCRAPE:
        ...
        try:
            ...
            page_links = [m[1] for m in matches]
            all_links.extend(page_links)
        except Exception as e:
            print(f"Failed to scrape {page_url}: {e}")
    return list(set(all_links)) # deduplicate

def find_best_link(filename, available_links):
    # Try to match keywords from filename
    # e.g. 2026_quarter_dollar_u.s._constitution_obverse.jpg -> 'constitution', 'obv'
    tags = filename.replace('.jpg', '').replace('_', ' ').replace('-', ' ').split()
    
    best_link = None
    best_score = 0
    for link in available_links:
        link_lower = link.lower()
        if '150x' in link_lower or '300x' in link_lower or 'icon' in link_lower:
            continue # skip thumbnails
        
        score = 0
        for tag in tags:
            if len(tag) > 3 and tag.replace('.', '') in link_lower:
                score += 1
                
        # Obverse/Reverse boost
        if 'obverse' in filename and ('obv' in link_lower or 'heads' in link_lower):
            score += 2
        if 'reverse' in filename and ('rev' in link_lower or 'tails' in link_lower):
            score += 2
    ...
    if best_link:
        if not best_link.startswith('http'):
            best_link = 'https://www.usmint.gov' + best_link
    return best_link

def apply_watermark(image_path):
    print(f"Watermarking {image_path}...")
    try:
...
def main():
    ...
    pending_mask = df['gcs_url'] == 'PENDING_UPLOAD'
    
    if pending_mask.sum() == 0:
        print("No pending uploads found.")
        return
        
    print(f"Found {pending_mask.sum()} pending coins.")
    links = []
    need_scrape = False
    
    # Check if we need to scrape (i.e. files don't exist locally)
    for idx, row in df[pending_mask].iterrows():
        local_path = os.path.join(BASE_DIR, row['filename'])
    ...
    if need_scrape:
        ...
        print(f"Scraped {len(links)} image links from all pages.")
    ...
    for idx, row in df[pending_mask].iterrows():
        ...
        # 1. Acquire Image
        if not os.path.exists(local_path):
            best_link = find_best_link(fname, links)
            if best_link:
                print(f"Downloading matched link for {fname}: {best_link}")
                try:
                    req = urllib.request.Request(best_link, headers=HEADERS)
                    with urllib.request.urlopen(req) as resp:
                        with open(local_path, 'wb') as out_f:
                            out_f.write(resp.read())
                except Exception as e:
                    print(f"Failed to download {best_link}: {e}")
                    continue
            else:
                print(f"Could not find a suitable image link for {fname}")
                continue
        ...
        # 3. Upload to GCS
        gcs_path = f"reference_library/2026_series/{fname}"
        print(f"Uploading {fname} to GCS at {gcs_path}")
        ...
        try:
            ...
            # 4. Update DataFrame
            df.at[idx, 'gcs_url'] = public_url
            df.at[idx, 'attribution'] = 'U.S. Mint'
            df.at[idx, 'license'] = 'Public Domain'
            updates_made += 1
        except Exception as e:
    ...
    if updates_made > 0:
        ...
        print(f"Successfully processed {updates_made} coins and updated CSV.")
    else:
        print("No new updates were processed.")
