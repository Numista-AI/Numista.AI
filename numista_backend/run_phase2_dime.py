...
BASE_DIR = r"C:\Users\ericd\Documents\MyVertexProject\Manual downloaded Coin Images\US Mint\Bulk_Scrape\Dime"
...
url = "https://www.usmint.gov/learn/coins-and-medals/circulating-coins/dime"
...
for link in links:
    ...
    # Restrict to Dime visuals
    if 'dime' not in filename.lower() and 'roosevelt' not in filename.lower() and 'mercury' not in filename.lower() and 'barber' not in filename.lower():
        continue
    ...
    try:
        ...
        # Deploy to GCS
        gcs_path = f"reference_library/bulk_programs/dime/{filename}"
        blob = bucket.blob(gcs_path)
        blob.upload_from_filename(local_path)
        public_url = f"https://storage.googleapis.com/{BUCKET_NAME}/{gcs_path}"
        
        program_tag = ""
        if "roosevelt" in filename.lower(): program_tag = "Roosevelt Dime"
        elif "mercury" in filename.lower() or "winged" in filename.lower(): program_tag = "Mercury Dime"
        elif "barber" in filename.lower(): program_tag = "Barber Dime"
        elif "seated" in filename.lower(): program_tag = "Seated Liberty Dime"
        elif "draped" in filename.lower(): program_tag = "Draped Bust Dime"
        ...
        csv_rows.append({
            ...
            'tags': program_tag if program_tag else "Dime",
            'gcs_url': public_url,
            'gcs_path': gcs_path,
...
print(f"Phase 2 and 3 executed for Dime page. {downloaded_count} physical assets processed.")
