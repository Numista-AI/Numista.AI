...
BASE_DIR = r"C:\Users\ericd\Documents\MyVertexProject\Manual downloaded Coin Images\US Mint\Bulk_Scrape\Dollar"
...
url = "https://www.usmint.gov/learn/coins-and-medals/circulating-coins/dollar-coins"
...
for link in links:
    ...
    # Restrict to Dollar visuals
    if 'dollar' not in filename.lower() and 'sacagawea' not in filename.lower() and 'anthony' not in filename.lower() and 'eisenhower' not in filename.lower():
        continue
    ...
    try:
        ...
        # Deploy to GCS
        gcs_path = f"reference_library/bulk_programs/dollar/{filename}"
        blob = bucket.blob(gcs_path)
        blob.upload_from_filename(local_path)
        public_url = f"https://storage.googleapis.com/{BUCKET_NAME}/{gcs_path}"
        
        program_tag = ""
        if "sacagawea" in filename.lower() or "native-american" in filename.lower(): program_tag = "Sacagawea / Native American Dollar"
        elif "innovation" in filename.lower(): program_tag = "American Innovation Dollar"
        elif "presidential" in filename.lower(): program_tag = "Presidential Dollar"
        elif "anthony" in filename.lower(): program_tag = "Susan B. Anthony Dollar"
        elif "eisenhower" in filename.lower(): program_tag = "Eisenhower Dollar"
        elif "peace" in filename.lower(): program_tag = "Peace Dollar"
        elif "morgan" in filename.lower(): program_tag = "Morgan Dollar"
        elif "trade" in filename.lower(): program_tag = "Trade Dollar"
        elif "seated" in filename.lower(): program_tag = "Seated Liberty Dollar"
        ...
        csv_rows.append({
            ...
            'tags': program_tag if program_tag else "Dollar Coin",
            'gcs_url': public_url,
            'gcs_path': gcs_path,
...
print(f"Phase 2 and 3 executed for Dollar page. {downloaded_count} physical assets processed.")
