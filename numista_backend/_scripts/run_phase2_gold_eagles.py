...
BASE_DIR = r"C:\Users\ericd\Documents\MyVertexProject\Manual downloaded Coin Images\US Mint\Bulk_Scrape\Gold_Eagles"
...
for link in links:
    ...
    # We want Gold Eagles (often listed as $10, $5, $20, eagles, etc)
    # The gold eagle page has various eagles. Let's filter loosely.
    if 'eagle' not in filename.lower() and 'gold' not in filename.lower() and 'coin' not in filename.lower():
        continue
    ...
    try:
        ...
        # Deploy to GCS
        gcs_path = f"reference_library/bulk_programs/gold_eagles/{filename}"
        ...
        # Parse names
        csv_rows.append({
            'denomination': 'Ten Dollars' if 'ten' in filename.lower() or '10' in filename else 'Varied',
...
print(f"Phase 2 and 3 executed for Gold Eagles. {downloaded_count} physical assets processed.")
