...
BASE_DIR = r"C:\Users\ericd\Documents\MyVertexProject\Manual downloaded Coin Images\US Mint\Bulk_Scrape\American_Eagle"
...
for link in links:
    ...
    if 'eagle' not in filename.lower():
        # Fallback check, maybe skip ones obviously unrelated
        pass
        
    gcs_path = f"reference_library/bulk_programs/american_eagle/{filename}"
    ...
    try:
        ...
        denom = 'Varied'
        if 'silver' in filename.lower():
            denom = 'One Dollar'
        elif 'platinum' in filename.lower():
            denom = 'One Hundred Dollars'
        elif 'palladium' in filename.lower():
            denom = 'Twenty Five Dollars'
        elif 'gold' in filename.lower():
            denom = 'Fifty Dollars' # Often 1oz vs fractional, Varied is safer but fifty is most common
...
print(f"Phase 2 and 3 executed for American Eagles. {downloaded_count} physical assets processed.")
