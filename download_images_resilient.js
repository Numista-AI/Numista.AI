    ...
    for (const file of files) {
        ...
        try {
            ...
            const imageData = await page.evaluate(async () => {
                ...
                const url = fullMediaLink ? fullMediaLink.href : (internalLink ? internalLink.href : null);
            ...
            if (imageData.base64) {
                fs.writeFileSync(dest, Buffer.from(imageData.base64, 'base64'));
                console.log(`  Success: ${file.name}`);
            } else {
                console.log(`  Failed: ${imageData.error}`);
                
                // Fallback: Take a screenshot of the image element if fetch fails
                console.log('  Attempting fallback screenshot...');
                const imgElement = await page.$('.fullImage img');
                if (imgElement) {
                    await imgElement.screenshot({ path: dest });
                    console.log(`  Success (Screenshot Fallback): ${file.name}`);
                } else {
                    console.log('  Fallback failed: Image element not found');
                }
            }
            await page.close();
            await sleep(2000); // Reduced sleep slightly for fetch approach
        } catch (e) {
